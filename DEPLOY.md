...
报告记录保存在 `.data/report-events.json`，可用 `REPORT_DATA_FILE` 指定其他路径。该目录不提交 Git。报告按访问者浏览器时区筛选当天，保留最近 31 天记录；启用前的活动不能追溯。浏览器刷新不会清除记录。服务重启可读取原文件，但云平台重新部署时若磁盘不持久化，历史会丢失；需要长期留存时应挂载持久磁盘并设置文件路径。
